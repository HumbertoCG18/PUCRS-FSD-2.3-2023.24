...
adicao_vetores:
    ...
    add_loop:
        ...
        lw $s5, ($s0)    # Carrega A[i] em $s5
        lw $s6, ($s1)    # Carrega B[i] em $s6
        
        add $s7, $s5, $s6  # Soma A[i] + B[i]
        sw $s7, ($s2)      # Armazena em C[i]
...
produto_escalar:
    ...
    scalar_loop:
        ...
        lw $s5, ($s2)    # Carrega C[i] em $s5
        lw $s6, ($s3)    # Carrega D[i] em $s6
